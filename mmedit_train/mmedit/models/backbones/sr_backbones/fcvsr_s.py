...
class FCVSR_SNet(nn.Module):
    def __init__(self, n_features=64, wiF=1.5, AC_Ks=3, ACNum=3, Freq_Inv=4, SCGroupN=4):
        ...
        self.feat_extract = nn.Sequential(nn.Conv2d(21, 7*self.n_feats, 3, 1, 1))
        self.lrelu = nn.PReLU() 
        ...
        self.conv_last0 = nn.Conv2d(self.n_feats, 3, 3, 1, 1, bias=True)
...
@BACKBONES.register_module()
class FCVSRNetbk(nn.Module):
    def __init__(self, n_features=64,wiF=1.5, AC_Ks=3, ACNum=6, Freq_Inv=8, SCGroupN=6):
        super(FCVSRNetbk, self).__init__()
        self.n_feats = n_features
        self.device = torch.device('cuda')
        self.wiF = wiF
        self.AC_Ks = AC_Ks
        ...
        self.feat_extract = nn.Sequential(nn.Conv2d(21, 7*self.n_feats, 3, 1, 1))
        self.lrelu = nn.PReLU() 
        self.MGAA = MGAA(dim=self.n_feats, wiF=self.wiF, AC_Ks=self.AC_Ks, ACNum=self.ACNum)
        ...
        self.rconcat1 = nn.Conv2d(self.n_feats, self.n_feats, 3, stride=2, padding=1, bias=True)
        self.rconcat2 = nn.Conv2d(self.n_feats, self.n_feats, 3, stride=2, padding=1, bias=True)
        self.recorb1 = SCNet(nf=self.n_feats, SCGroupN=self.SCGroupN) 
        self.recorb0 = nn.Conv2d(self.n_feats, self.n_feats, 3, 1, 1, bias=True)
        #### upsampling
        #### fea pyramid fuse 
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch_size,-1, height, width)    
        # Feature extraction  
        # print('x',x.shape,shortcut.shape)
        x1 = self.feat_extract(x)
        sam_features = x1 
        f1 = sam_features[:,0:3*self.n_feats,:,...]
        f2 = sam_features[:,3*self.n_feats:4*self.n_feats,:,...]
        ...
        cat_feat = torch.cat([MGAA_outs1,f2,MGAA_outs3],dim=1)
        MGAA_outs2, MV_save, Feat_save = self.MGAA(cat_feat)
        # cv2.imwrite("/share22/home/zhuqiang/zhuqiang/CVSR_train/alignimg/vizoffset_forback.png",MV_save)
        # plt.figure()
        # plt.imshow(Feat_save)
        # plt.axis('off')
        # fig = plt.gcf()
        ...
        #     if i == frames//2:
        #         base_out.append(F.interpolate(shortcut[:,i,:,:,:], scale_factor=4, mode='bilinear') + out)
        #     else:
        #         base_out.append(F.interpolate(shortcut[:,i,:,:,:], scale_factor=4, mode='bilinear'))
        # base = self.img_upsample(shortcut) 
        # print('out',out.shape, base.shape)
        # 
        # print('out',out.shape, base.shape)
        # out = torch.stack(base_out, dim=1)
...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        # print('corr',corr.shape, coords_lvl.shape)  #  ([8320, 1, 128, 65]) torch.Size([8320, 9, 9, 2])

        corr = bilinear_sampler(corr, coords_lvl)
        corr = corr.view(batch, h1, w1, -1)
        out = corr # out_pyramid.append(corr)
        # out = torch.cat(out_pyramid, dim=-1)
        # print('out',out.shape)  # ([1, 128, 65, 81])
    ...
    def corr(fmap1, fmap2):
        ...
        corr = fmap1 * fmap2
        # print('corr',fmap1.transpose(1,2).shape, fmap2.shape, corr.shape)  # torch.Size([1, 8320, 128]) torch.Size([1, 128, 8320])
...
class MGAAbk(nn.Module):
    ...
    def forward(self, x, flow_init=None):
        ...
        if flow_init is not None:
            coords1f = coords1f + flow_init
            coords1b = coords1b + flow_init
...
class ConvBlk(nn.Module):
    def __init__(self, dim=64, index=1, res_scale=1.0):
        ...
        self.conv2 = nn.Conv2d(dim, 4, self.k_size, stride=1, padding=(self.k_size)//2, bias=False)
        self.relu = nn.PReLU() # nn.ReLU(inplace=True)
        self.CA = CALayer(4, 1, bias=False)
...
class MGAA(nn.Module):
    ...
    def forward(self, x):
        ...
        # MV_save = flow_to_image(MV_save, None,convert_to_bgr=False)
        # MV_save = MV_save[:, :, [2,1,0]]
        # cv2.imwrite("/share22/home/zhuqiang/zhuqiang/mmedit04/alignimg/vizoffset_forback.png",MV_save)
        ...
        out = self.conv3(torch.cat([Aligned_x2f,Aligned_x2b],dim=1)) #  + x2
        # out = Aligned_x2f + Aligned_x2b + x2
        Feat_save = np.zeros([H,self.ACNum*W,1])
        # Feat_save[:,0:W,:] = featuremap_visual(x1,num_ch=29)
        # Feat_save[:,W:2*W,:] = featuremap_visual(x3,num_ch=29)
        # Feat_save[:,2*W:3*W,:] = featuremap_visual(Aligned_x2f,num_ch=29)
        ...
        # plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
        # plt.margins(0,0)
        # plt.savefig("/share22/home/zhuqiang/zhuqiang/mmedit04/alignimg/MMGA_vizfeat.png", bbox_inches='tight', transparent=True, dpi=100)
        ...
        if self.mode == "ideal":
            ...
            for n in range(self.channel_num):
                ...
                for i in range(h_list.shape[0]):
                    pf[i, :] = h_list[i] + w_list
                pf = np.sqrt(pf)
                pf = 1/(1+np.power((pf/(length_interval*(n+1))), 2*(n+1)))
                pf = torch.from_numpy(pf).float().unsqueeze(0)
                if n == 0:
                    pass
                else:
...
#     def forward(self, x, x_before=None):
#         if x_before == None:
...
class MultiFreq_Refinment(nn.Module):
    ...
    def forward(self, x):
        ...
        # plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
        # plt.margins(0,0)
        # plt.savefig("/share22/home/zhuqiang/zhuqiang/mmedit04/alignimg/MFFR_vizbefor.png", bbox_inches='tight', transparent=True, dpi=100)       
        ...
        # plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
        # plt.margins(0,0)
        # plt.savefig("/share22/home/zhuqiang/zhuqiang/mmedit04/alignimg/MFFR_vizenh.png", bbox_inches='tight', transparent=True, dpi=100)       
