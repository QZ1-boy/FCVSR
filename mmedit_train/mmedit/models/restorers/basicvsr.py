...
class BasicVSR(BasicRestorer):
    ...
    def evaluate(self, output, gt, output_pre, gt_pre):
        """Evaluation function.
        ...
        for metric in self.test_cfg.metrics:
            if output.ndim == 5:  # a sequence: (n, t, c, h, w)
                ...
                output_img_pre = tensor2img(output_pre)
                gt_img_pre = tensor2img(gt_pre)
                value = self.allowed_metrics[metric](
                    output_img, gt_img, output_img_pre, gt_img_pre, crop_border, convert_to=convert_to)
    ...
    def forward_test(self,
        ...
        if gt is not None and gt.ndim == 4:
            ...
            if self.check_if_mirror_extended(lq):  # with mirror extension
                ...
                # output = output[:, t // 2]   # for basicvsrpp
                output = output    # for fcvsr
        
        # print('iteration',iteration)
        # print('[output gt]',output.shape, gt.shape)
        if self.gt_pre is None:
            self.gt_pre = gt
            self.outputg_pre = output
        
        
        if self.test_cfg is not None and self.test_cfg.get('metrics', None):
            assert gt is not None, (
                'evaluation with metrics must have gt images.')
            results = dict(eval_result=self.evaluate(output, gt, self.outputg_pre, self.gt_pre))
        else:
