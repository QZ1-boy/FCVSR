...
from .fcvsr_restorer import FCVSRRestorer 
...
class FCVSR(FCVSRRestorer):
    """FCVSR model for video super-resolution.
    Args:
    ...
    def train_step(self, data_batch, optimizer):
        ...
        if self.step_counter == 0:
            if self.train_cfg is None:
                raise KeyError('train_cfg must contain.')
    ...
    def forward_test(self,
        ...
        #     h1 = 720
        # lq = lq[:,:,:,:(h1//4)*4,:(w1//4)*4]
        # print('gt',lq.shape, gt.shape)
        output = self.generator(lq)
        if gt.ndim == 5:
            gt = gt[:, n1//2, :, :, :].squeeze(1)
        # print('gt',output.shape, gt.shape)
        # gt = gt[:,5//2,:,:,:].squeeze(1)
        # b2,  c2, h2, w2 = gt.shape
        # gt = gt[..., :(h2//16)*16,:(w2//16)*16]
        # print('lq test',lq.shape, output.shape, gt.shape)
...
class MultiFreq_Refinment(nn.Module):
    def __init__(self, dim, Freq_Inv=8, mode="ideal", freq_order="l2h", bias=False, add_channel=None):
    ...
    def forward(self, x):
        ...
        for i in range(0,self.Freq_Inv):
            ...
            if i == 0:
                ...
                freq_o = self.DivEnh_block[i](freq[i],freq[i-1])
