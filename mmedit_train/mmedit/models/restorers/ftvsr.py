...
class FTVSR(BasicRestorer):
    ...
    def __init__(self, mid_channels=64, num_blocks=72, stride=4, keyframe_stride=3,spynet_pretrained=None, 
...
class FTT(nn.Module):
    ...
    def __init__(self, dct_kernel=(8,8), d_model=144, n_heads=8):
        super().__init__()
        self.dct_kernel = dct_kernel
        self.dct = dct_layer(in_c=3, h=dct_kernel[0], w=dct_kernel[1])
        self.rdct = reverse_dct_layer(out_c=3, h=dct_kernel[0], w=dct_kernel[1])

        self.conv_layer1 = nn.Conv2d(192, 144, 1, 1, 0, bias=True)
        self.feat_extractor = ResidualBlocksWithInputConv(144, 144, 3)
        self.resblocks = ResidualBlocksWithInputConv(144*2, 144, 3)
        self.fusion = nn.Sequential(
            nn.Conv2d(3 * 144, 144, 1, 1, 0, bias=True),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv2d(144, 144, 1, 1, 0, bias=True)
        )
        
        self.ftta = FTTA_layer(channel=144, d_model=d_model, n_heads=n_heads)

        self.conv_layer2 = nn.Conv2d(144, 192, 1, 1, 0, bias=True)
    ...
    def forward(self, bicubic_imgs, high_frequency_imgs, flows, padiings, to_cpu=False):
        ...
        dct_bic_fea = self.feat_extractor(self.conv_layer1(dct_bic)).view(n, t, 144, h//self.dct_kernel[0], w//self.dct_kernel[1])
        dct_hfi_fea = self.feat_extractor(self.conv_layer1(dct_hfi)).view(n, t, 144, h//self.dct_kernel[0], w//self.dct_kernel[1])
...
class FTT_encoder(nn.Module):
    def __init__(self, channel=192, d_model=144, n_heads=8, num_layer=3):
        super().__init__()
...
class FTTA_layer(nn.Module):
    ...
    def __init__(self, channel=192, d_model=144, n_heads=8, patch_k=(8,8), patch_stride=8):
        super().__init__()
