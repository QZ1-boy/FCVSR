...
def psnr(img1, img2,  img1_pre=None, img2_pre=None, crop_border=0, input_order='HWC', convert_to=None):
...
def ssim(img1, img2, img1_pre=None, img2_pre=None, crop_border=0, input_order='HWC', convert_to=None):
    ...
    img1 = reorder_image(img1, input_order=input_order)
    img2 = reorder_image(img2, input_order=input_order)
    # print('ssim',img1.shape, img2.shape)
...
def tOF(pred_img_cur, true_img_cur, pred_img_pre, true_img_pre, crop_border=0, input_order='HWC', convert_to=None):

    assert pred_img_cur.shape == true_img_cur.shape, (
        f'Image shapes are different: {pred_img_cur.shape}, {true_img_cur.shape}.')
    if input_order not in ['HWC', 'CHW']:
        raise ValueError(
            f'Wrong input_order {input_order}. Supported input_orders are '
            '"HWC" and "CHW"')
    if true_img_cur.shape != true_img_pre.shape:
        true_img_pre = true_img_cur
        pred_img_pre = pred_img_cur

    pred_img_cur = reorder_image(pred_img_cur, input_order=input_order)
    true_img_cur = reorder_image(true_img_cur, input_order=input_order)
    pred_img_pre = reorder_image(pred_img_pre, input_order=input_order)
    true_img_pre = reorder_image(true_img_pre, input_order=input_order)

    # print('true_img_cur',true_img_cur.shape, true_img_pre.shape)

    if isinstance(convert_to, str) and convert_to.lower() == 'y':
        pred_img_cur, true_img_cur = pred_img_cur.astype(np.float32), true_img_cur.astype(np.float32)
        pred_img_pre, true_img_pre = pred_img_pre.astype(np.float32), true_img_pre.astype(np.float32)
        pred_img_cur = mmcv.bgr2ycbcr(pred_img_cur / 255., y_only=True) * 255.
        true_img_cur = mmcv.bgr2ycbcr(true_img_cur / 255., y_only=True) * 255.
        pred_img_cur = np.expand_dims(pred_img_cur, axis=2)
        true_img_cur = np.expand_dims(true_img_cur, axis=2)

        pred_img_pre = mmcv.bgr2ycbcr(pred_img_pre / 255., y_only=True) * 255.
        true_img_pre = mmcv.bgr2ycbcr(true_img_pre / 255., y_only=True) * 255.
        pred_img_pre = np.expand_dims(pred_img_pre, axis=2)
        true_img_pre = np.expand_dims(true_img_pre, axis=2)
    elif convert_to is not None:
        raise ValueError('Wrong color model. Supported values are '
                         '"Y" and None')
    # print('true_img_cur 1',true_img_cur.shape, true_img_pre.shape)
    
    if crop_border != 0:
        pred_img_cur = pred_img_cur[crop_border:-crop_border, crop_border:-crop_border, None]
        true_img_cur = true_img_cur[crop_border:-crop_border, crop_border:-crop_border, None]
        pred_img_pre = pred_img_pre[crop_border:-crop_border, crop_border:-crop_border, None]
        true_img_pre = true_img_pre[crop_border:-crop_border, crop_border:-crop_border, None]
