...
class CDVL_sideInfo_Dataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None, QP=32, only_I_frame=True, random_start=False,
        ...
        self.SideInfo_dir_prefix22 = SideInfo_dir_prefix + 'QP' + '22' + '/LD_'
        ...
        self.MVl1s_ = np.zeros([len(self.data_path_details), 32, 270, 480, 3], dtype = np.uint8)
        ...
        for d_i in range(len(self.data_path_details)):
            ...
            for f_i in range(32):
                ...
                # if self.pred_flag:
                #     pred_tmp = io.imread(pred_f_name)
                #     self.UFs_[d_i, f_i, :, :] = pred_tmp
    ...
    def __getitem__(self, idx):
        ...
        if self.mv_flag:
            ...
            if self.only_1_GT:
                ...
                # mvl1_name = side_paths + 'MV_l1/' + img_idx + '_mvl0.npy'
                # mvl1_tmp = np.clip(np.load(mvl1_name), -128, 127).astype(np.int8)
                mvl1s.append(mvl1)
            else:
                print("??")
        ...
        if self.res_flag:
            ...
            # res_name = side_paths + 'Residue/' + first_poc + '_res.npy'
            # one_tmp.append(res_name)
            # res_tmp = np.clip(np.load(res_name), -128, 127).astype(np.int8)
        else:
            res_s = None
