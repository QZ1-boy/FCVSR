...
def eval_seq(tst_list, gt_list, methods_name = "J", coding_cfg = "LD", testing=True, cal_metric=True):
    if testing:
        QP = 32
        # for QP in ['37']:  #  ['27', '27', '32', '37']
        model = GShiftNet()
        print("number of model parameters:", sum([np.prod(p.size()) for p in model.parameters()]))
        model_path = '/share3/home/zqiang/CVSR_train/training_results/train_LD_freqCVSR_32/ckpt/epoch-3000.pth'  # 10000 18000  16400
        ...
        for img_set in tst_list:
            ...
            save_path = './results_evl/%s_freqCVSR_QP%s_%s/%s/' % (coding_cfg, QP, methods_name, img_set)
    ...
    if cal_metric:
        QP = 32
        f1 = open("./log/%s_%s_freqCVSR_QP%s.txt" % (coding_cfg, methods_name, QP), 'a+')
        ave_psnr_l = []
        ave_ssim_l = []
        for one_t, one_gt in zip(tst_list, gt_list):
            ...
            psnr, ssim, psnr_n, ssim_n = cal_psnr_ssim(
                        './results_evl/%s_freqCVSR_QP%s_%s/' % (coding_cfg, QP, methods_name),
                        [one_t],
                        [one_gt],
                        './test_data/gt_Y/')
            psnr_s.append(psnr)
