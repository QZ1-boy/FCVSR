...
import numpy as np
import argparse
from arch.OtherMethod_arch import EDVRNet
...
def eval_seq(tst_list, gt_list, coding_cfg = "LD", testing=True):
    if testing:
        ...
        model_path = '/share3/home/zqiang/mmediting0406/mmediting-master/work_dirs/edvrl_c128b40_8x8_lr2e-4_600k_CVCP_LD_QP22/iter_5.pth' 
        # torch.load_state_dict(torch.load(model_path, map_location='cpu')['state_dict'])
        ...
        for img_set, one_gt in zip(tst_list, gt_list):
            ...
            for _, _, f in os.walk(tmp_path):
                ...
                for i in range(len(f)):
                    ...
                    lrs = torch.unsqueeze(torch.cat(input_imgY, 0).repeat(1, 3, 1, 1).to(device), 0)
                    # print('[lrs]', lrs.shape)
                    with torch.no_grad():
                        strT = time.time()
                        cur_sr = model(lrs)
                    ...
                    print(i,'...', end="\r")
                FPS = len(f) / Sumtime 
                print('EDVR at', one_gt, 'FPS:', FPS)
                f1 = open("./FPS/log_EDVR_FPS.txt", 'a+')
