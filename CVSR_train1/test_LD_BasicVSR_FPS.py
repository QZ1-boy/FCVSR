...
import numpy as np
import argparse
from arch.OtherMethod_arch import BasicVSRNet
...
def eval_seq(tst_list, gt_list, coding_cfg = "LD", testing=True):
    if testing:
        QP = 22
        model = BasicVSRNet(mid_channels=64,
                 num_blocks=30,
                 spynet_pretrained='/share3/home/zqiang/mmediting0406/mmediting-master/spynet_20210409-c6c1bd09.pth')
        print("number of model parameters:", sum([np.prod(p.size()) for p in model.parameters()]))
        model_path = '/share3/home/zqiang/mmediting0406/mmediting-master/work_dirs/basicvsr_cvcp_LD_QP22/latest.pth' 
        ...
        for img_set, one_gt in zip(tst_list, gt_list):
            ...
            for _, _, f in os.walk(tmp_path):
                ...
                for i in range(len(f)):
                    ...
                    print(i,'...', end="\r")
                FPS = len(f) / Sumtime 
                print('BasicVSR at', one_gt, 'FPS:', FPS)
                f1 = open("./FPS/log_BasicVSR_FPS.txt", 'a+')
