...
trackers = ['FTVSR++','EDVR-L','CD-VSR','BasicVSR','IconVSR', 'BasicVSR++','FCVSR-S','FCVSR', 'FCVSR-ETC', '5M','10M','15M','20M','25M']  # mothod name
trackers_ = ['#Param.']
speed = np.array([0.27,2.02,5.16,0.93,0.506,0.74,5.28,2.39,0.34,3-0.15,3.4,4.12,5.125,6.14])  # second (ms)
...
performance = np.array([25.09,24.87,24.87,24.99,24.99,25.05,24.93,25.20,25.20,24.57,24.57,24.57,24.57,24.57,24.57])  # psnr (dB)
...
'powderblue','lightpink', 'pink', 'pink', 'whitesmoke','gainsboro','lightgrey','silver','darkgray']
...
bax.text(speed[0]-0.70, performance[0]+0.10, trackers[0],   fontsize=14, color='k')
bax.scatter(speed[0], performance[0], c='dimgray', s=2)

# bax.quiver([speed[0]-0.35],[performance[0]+0.08], [0.47],[-0.11],angles='xy',scale_units='xy',width=0.003,scale=1.5,color="black")
...
bax.text(speed[3]-0.79, performance[3]-0.15, trackers[3],   fontsize=14, color='k') 
bax.scatter(speed[3], performance[3], c='dimgray',s=2) 

bax.quiver([speed[3]-0.06],[performance[3]-0.098], [0.20],[0.27],angles='xy',scale_units='xy',width=0.003,scale=3.2,color="black")

bax.scatter(speed[4], performance[4], c=colors1[1], s=8.70*8.70*100/3, marker='o',linewidths=0.5, edgecolors='white')
bax.text(speed[4]-0.98, performance[4]-0.09, trackers[4],   fontsize=14, color='k') 
bax.scatter(speed[4], performance[4], c='dimgray',s=2) 

# bax.quiver([speed[4]-0.50],[performance[4]-0.12], [1.30],[0.30],angles='xy',scale_units='xy',width=0.003,scale=2.8,color="black")
