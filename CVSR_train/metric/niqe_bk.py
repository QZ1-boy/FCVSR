...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    himg,wimg = img.shape[0:2]
    # print('img',img.shape)
    img2 = np.array(Image.fromarray(img).resize((himg//2,wimg//2)))
...
def cal_niqe_seq(seq_path):
    ...
    for root, dirs, files in os.walk(seq_path):
        ...
        for j in range(len(dirs)):
            dirs.sort()
            dir_path = root + '/' + dirs[j] + '/' 
            for _, _, files in os.walk(dir_path):
                files.sort()
                seq_ave_niqe = 0
                niqe_seq = 0
                for fm_i in range(len(files)):
                    idx ="%08d" % fm_i
                    img_name = dir_path + idx + '.png' 
                    res_img = Image.open(img_name)                  
                    res_img = np.array(res_img)
                    f_niqe = cal_niqe(res_img)
                    # f_niqe = iqa_metric(img_name)
                    niqe_seq += f_niqe
                    print('seq: %s (%d/%d) ... ' % (dirs[j], fm_i, len(files)), end='\r')                   
                seq_ave_niqe += niqe_seq / len(files)
                niqe_seq_list.append(seq_ave_niqe)
                msg = '%s Average NIQE: %.4f' % (dirs[j], seq_ave_niqe)
                print(msg)
    niqe_all = np.sum((niqe_seq_list))/len(dirs)
    msg = 'VideoLQ Average NIQE: %.4f' % (niqe_all)
    print(msg)
...
if __name__ == "__main__":
    ...
    ref = np.array(Image.open('/share3/home/zqiang/CVSR_train/metric/test_imgs/bikes.bmp').convert('LA'))[:,:,0] # ref
    dis = np.array(Image.open('/share3/home/zqiang/CVSR_train/metric/test_imgs/bikes_distorted.bmp').convert('LA'))[:,:,0] # dis

    print('NIQE of ref bikes image is: %0.3f'% niqe(ref))
    print('NIQE of dis bikes image is: %0.3f'% niqe(dis))

    ref = np.array(Image.open('/share3/home/zqiang/CVSR_train/metric/test_imgs/parrots.bmp').convert('LA'))[:,:,0] # ref
    dis = np.array(Image.open('/share3/home//zqiang/CVSR_train/metric/test_imgs/parrots_distorted.bmp').convert('LA'))[:,:,0] # dis
