...
class CDVL_Dataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None, QP=32, only_I_frame=True, random_start=False, qp_flag=False, max_len=8, only_1_GT=False, 
        ...
        self.HR_imgs_ = np.zeros([len(self.data_path_details), 32, 1080, 1920], dtype = np.uint8)
...
class CDVL_Dataset_ETC(Dataset):
    """compressed yuv with side info dataset"""
    def __init__(self, csv_file, transform=None, QP=32, only_I_frame=True, random_start=False, qp_flag=False, max_len=13, only_1_GT=False, 
                 need_bi_flag=False,
                 HR_dir="/share3/home/zqiang/CVCP/Uncompressed_HR/",  #   "/data/cpl/sideInfoVSRdata/VC600_GT_grey_npy/"
                 LR_dir_prefix="/share3/home/zqiang/CVCP/Decoded_LR/LD/",    #  "/data/cpl/sideInfoVSRdata/LR/LD/"
                 SideInfo_dir_prefix="/share3/home/zqiang/CVCP/Coding_Priors/LD/"):   #   "/data/cpl/sideInfoVSRdata/side/LD/"
        ...
        self.dir_all = []
        # print('len(self.data_path_details)',len(self.data_path_details))
        ####
        self.LR_imgs_ = np.zeros([len(self.data_path_details), 32, 270, 480], dtype = np.uint8)
        self.HR_imgs_ = np.zeros([len(self.data_path_details), 32, 1080, 1920], dtype = np.uint8)
        self.QPs = np.zeros([len(self.data_path_details), 32], dtype = np.int8)
        ...
        for d_i in range(len(self.data_path_details)):
            ...
            for f_i in range(32):
                ...
                hr_img_tmp = io.imread(hr_img_name)
                self.HR_imgs_[d_i, f_i, :, :] = hr_img_tmp
                ...
                seq_tmp.append(one_tmp)
            self.dir_all.append(seq_tmp)
            # print('self.dir_all',self.dir_all)
    ...
    def __getitem__(self, idx):
        ...
        if self.only_I_frame:
            ...
            if self.random_start:
                first_poc = random.randint(0,19)   #  25
            else:
                first_poc = random.randint(0,13) * 2

        lr_imgs = self.LR_imgs_[idx, first_poc:first_poc+13, :, :]  #  13
        hr_imgs = self.HR_imgs_[idx, first_poc:first_poc+13, :, :]  #  13
        center_idx = self.max_len // 2 + first_poc
        
        if self.qp_flag:
            qp = self.QPs[idx, first_poc:first_poc+13]
        else:
            qp = None
        ...
        # print('lr_imgs',lr_imgs.shape,hr_imgs.shape)
        sample = {'lr_imgs': lr_imgs,
                  'hr_imgs': hr_imgs,
                  'qp': qp,
                  'lrbi': lr_bi_imgs}
