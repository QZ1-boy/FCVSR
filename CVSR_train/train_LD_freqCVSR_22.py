...
from opt.loss import CharbonnierLoss, MSELoss
...
def parse_args():
    ...
    parser.add_argument('--lr', default=0.5*1e-5, type=float)
...
def eval_seq(tst_list, gt_list, epoch, coding_cfg = "LD", testing=True, cal_metric=True):  
    if testing:
        ...
        # sys.exit(0)
        model_path = './training_results/train_LD_freqCVSR_%s/ckpt/epoch-%s.pth' % (args.qp, epoch)
        ...
        for img_set in tst_list:
            ...
            save_path = './train_evl_results/%s_QP%s_freqCVSR/%s/' % (coding_cfg, QP, img_set)
    ...
    if cal_metric:
        f1 = open("./train_evl_results/log/%s_our_freqCVSR_%s.txt" % (coding_cfg, QP), 'a+')
        for one_t, one_gt in zip(tst_list, gt_list):
            psnr_s = []
            ssim_s = []
            QP = args.qp
            psnr, ssim, psnr_n, ssim_n = cal_psnr_ssim(
                        './train_evl_results/%s_QP%s_freqCVSR/' % (coding_cfg, QP),
                        [one_t],
                        [one_gt],
                        './test_data/gt_Y/')
            psnr_s.append(psnr)
...
def train(args, model):
    ...
    side_dataset = CDVL_Dataset(csv_file='./misc/sequences_list.csv',
                                         transform=composed,
                                         only_I_frame=False,
                                         random_start=True,
    ...
    # optimizer
    milestones = [2000, 6000, 10000, 120000]
...
def main(args):
    ...
    model.load_state_dict(torch.load('/share3/home/zqiang/CVSR_train/training_results/train_LD_freqCVSR_22/ckpt/epoch-7800.pth', map_location='cpu'), strict=False)
    # model_dict = model.state_dict()
    # pretrained_dict = torch.load('/share3/home/zqiang/CVSR_train/training_results/train_LD_freqCVSR_22/ckpt/epoch-1600.pth', map_location = 'cpu')
